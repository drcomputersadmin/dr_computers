...
class Install extends CI_Controller {
  ...
  function finalizing_setup($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'setup_admin') {
      $admin_data['username']       = $this->input->post('name');
      $admin_data['email']      = $this->input->post('email');
      $admin_data['password']   = sha1($this->input->post('password'));

      $this->load->database();

      $this->db->insert('db_users', $admin_data);

      $data['site_name']  = $this->input->post('system_name');
      
      $this->db->update('db_sitesettings', $data);
  ...
  function success($param1 = '') {
    ...
    $this->load->database();
    $admin_email = $this->db->get_where('db_users', array('role_id' => 1))->row()->email;
